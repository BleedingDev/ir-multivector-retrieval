...
Numbers below are **rebenched under L10's harness discipline** ([commit a43668f](.codex/deep-research-zig-2026-05-01.txt) + L12's `smp_allocator` flip in [03ddb1d](https://github.com/anthropic/ir-multivector-retrieval/commit/03ddb1d)) on Apple M5 (10 cores, Darwin 25.3.0, ReleaseFast). Per-stage and total numbers report the **median across 6 full-build runs**. Stage-level p99 is wide because `pq.train` dominates and is sensitive to Apple-Silicon QoS scheduling — see [`REBENCH_SUMMARY.md`](.codex/bench-outputs/REBENCH_SUMMARY.md) for raw run logs and the variance discussion.

| Phase | Time (median, 6 runs) | Notes |
|---|---:|---|
| Metal encode (pylate, fp32, batch=64) | **48 min** | GPU-bound; FP16 + larger batch is 4-6× ahead. Encode-side number not part of this rebench (CPU-side build only). |
| `tac.clusterFlat` (parallel) | 8.5 s (range 8.0–13.6 s) | comptime-specialized distance kernels ([plan-11](.codex/plans/post-hackathon-11-vec-simd-specialization.plan.md)) |
| residuals + norms (parallel) | 54 ms | |
| **`pq.train` (parallel + work-stealing outer dispatch + comptime-specialized kernels)** | **176.9 s (range 140–390 s)** | atomic-counter outer dispatch ([plan-10](.codex/plans/post-hackathon-10-pq-workstealing.plan.md)) plus comptime-specialized `vec.l2sq` for hot dims ([plan-11](.codex/plans/post-hackathon-11-vec-simd-specialization.plan.md)). Dominant build stage; high cross-run variance on M5 (cumulative thermal load between back-to-back runs visibly inflates Lloyd-iter cost in the slowest 1/6 run). Index byte-equal across n_threads ∈ {1, 4, 10} preserved. |
| `pq.encode` (parallel) | 2.4 s | |
| **`hnsw.build` (parallel, 10 threads)** | **6.7 s (range 5.5–13.0 s)** | chunked deferred-commit, paper-strict deterministic CSR ([plan-06](.codex/plans/post-hackathon-06-hnsw-parallel.plan.md)) |
| serialise | 0.32 s | |
| **Build total (10 threads)** | **3 min 15 s median (best 2 min 37 s, worst 6 min 54 s; n=6)** | work-stealing pq.train ([plan-10](.codex/plans/post-hackathon-10-pq-workstealing.plan.md)) + comptime-specialized distance kernels ([plan-11](.codex/plans/post-hackathon-11-vec-simd-specialization.plan.md)). **Regression vs prior README: the old "1 min 23 s" headline was measured with `DebugAllocator` and no warm-up/cooldown; under the L10 protocol (`smp_allocator`, warm-up cells discarded, cooldown between timed cells, self-describing run header) this build is 2.0–2.5× slower than that prior claim. Index `.tac` is still byte-equal across thread counts within the same image; the cross-run wall-time variance is in scheduling, not in output bytes.** |
| **Search latency** (κ_c=80, κ_d=1000, α=null, single-thread per paper §9) | **p50 ≈ 100 ms / query, p99 ≈ 110–150 ms (quiet machine)**; p50 drifts to 170–340 ms under concurrent thermal load. n=90 samples per run × 7 runs. | The old "~54 ms / query" claim is not reproducible under the L10 protocol; the closest stable-floor we can measure is ~98 ms p50 on a cool, idle M5 with `BENCH_COOLDOWN_MS=200`. See [`REBENCH_SUMMARY.md`](.codex/bench-outputs/REBENCH_SUMMARY.md) and the per-iter CSV artifacts in `.codex/bench-outputs/`. |
| **Index size** | **75.75 MB** | for 26,678 Jira tickets (unchanged from prior measurement). |
...
> **Pre-rebench numbers below** (the 1:34 / 1:23 / 84.8 s / 74.9 s / 115.8 s / 97.1 s / 17.0 s / 2.8 s baselines and deltas in this paragraph) were measured under the **old protocol** — `DebugAllocator` and no warm-up/cooldown — before audit finding L10. The *direction and shape* of these wins (work-stealing > static chunker, comptime-specialized > generic kernels) is preserved post-rebench because byte-equality of the index across n_threads is unchanged; the *absolute seconds* in this paragraph have not been rebenched under the L10 protocol and are kept here for historical context. The headline "Build total" / "Search latency" / per-stage figures in the table above are the trustworthy current numbers.
